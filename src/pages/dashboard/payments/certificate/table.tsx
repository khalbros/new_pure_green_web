...
import { deleteCertPaymentAction } from "../../../../store/actions/finance"
...
const PaymentCertTable = () => {
  ...
  const { data, error, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      return fetchData("/payment/all/certificate").then((res) => res.data)
    },
  })
  ...
  const exportTableData = () => {
    return payments?.map((payment) => ({
      ...
      Cooperative: (payment?.cooperative as ICooperative)?.name,
      Chairman: (payment?.cooperative as ICooperative)?.chairman,
      Warehouse: (payment?.warehouse as IWarehouse)?.name,
  ...
  const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const result = (data as IPayment[])?.filter(
      (payment) =>
        (payment?.cooperative as ICooperative)?.name
          ?.toLowerCase()
          .includes(value.toLowerCase()) ||
        (payment.paid_by as IUser).name
          ?.toLowerCase()
          .includes(value.toLowerCase()) ||
        (payment?.cooperative as ICooperative)?.chairman
          ?.toLowerCase()
          .includes(value.toLowerCase()) ||
        (payment?.warehouse as IWarehouse)?.name
          ?.toLowerCase()
          .includes(value.toLowerCase()) ||
        payment?.status?.toLowerCase().includes(value.toLowerCase()) ||
  ...
  const handleDelete = (payment: IPayment) => {
    dispatchAction(
      deleteCertPaymentAction({ ...payment }, () => {
        queryClient.invalidateQueries(["certificate", "list"], { exact: true })
    ...
    <>
      <QueryResult
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            onClick={() => generateExcelFile(exportTableData(), "certificates")}
              ...
              <tbody>
                {currentItems?.map((payments, key) => (
                  <tr
                    ...
                    <td className="p-3">
                      {(payments?.cooperative as ICooperative)?.chairman}
                    </td>
                    <td className="p-3">
...
export default PaymentCertTable

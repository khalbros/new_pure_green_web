...
export const updateRegistrationPaymentAction = (
  ...
  try {
    const response = await http.patch(`/payment/registration/${payment._id}`, {
      ...payment,
    })
    if (!response.data) {
      dispatch(failed(response.statusText))
...
// Certificate
export const certPaymentAction = (
  payment: FormData,
  callback: () => void
) => async (dispatch: Dispatch<any>) => {
  if (!payment.has("cooperative")) {
    dispatch(failed("Please enter cooperative"))
    return
  }
  if (!payment.has("amount_paid")) {
    dispatch(failed("Please enter amount"))
  ...
  try {
    const respone = await http.post("/payment/certificate", formData)
    if (!respone.data) {
      dispatch(failed(respone.statusText ?? respone))
      return
    }
...
export const updateCertPaymentAction = (
  ...
  try {
    const response = await http.patch(`/payment/certificate/${payment._id}`, {
      ...payment,
    })
    if (!response.data) {
      dispatch(failed(response.statusText))
...
export const deleteCertPaymentAction = (
  payment: ICertificate,
  callback: () => void
) => async (dispatch: Dispatch<any>) => {
  if (!payment._id) {
  ...
  try {
    const response = await http.delete(`/payment/certificate/${payment._id}`)
    if (!response.data) {
      dispatch(failed(response.statusText))
      return
    }
